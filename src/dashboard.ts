...
import type { CodexSession, DailyUsageBucket, ModelUsage, RateLimitBucket, UsageSnapshot } from "./types";
...
function formatNumber(value?: number): string { return typeof value === "number" ? new Intl.NumberFormat().format(value) : "—"; }
function formatReset(timestamp?: number): string { return timestamp ? new Date(timestamp * 1000).toLocaleString() : "—"; }
function formatDate(timestamp?: number): string { return timestamp ? new Date(timestamp * 1000).toLocaleString() : "Unknown date"; }
function getBuckets(snapshot: UsageSnapshot): RateLimitBucket[] {
  ...
  return snapshot.rateLimits?.rateLimits ? [snapshot.rateLimits.rateLimits] : [];
}
...
function radar(models: ModelUsage[]): string {
  ...
  if (!data.length) return `<div class="empty">Model data will appear when stored sessions expose model and token metadata.</div>`;
  ...
  const rings = [0.25, 0.5, 0.75, 1].map((ratio) => {
    const pts = Array.from({ length: count }, (_, i) => point(i, radius * ratio).join(",")).join(" ");
...
function modelBars(models: ModelUsage[]): string {
  const max = Math.max(1, ...models.map((item) => item.tokens || item.sessions));
  return models.slice(0, 8).map((item) => {
    const width = Math.max(4, Math.round(((item.tokens || item.sessions) / max) * 100));
  ...
  }).join("") || `<div class="empty">No model distribution is available yet.</div>`;
}

function sessionRows(sessions: CodexSession[]): string {
  if (!sessions.length) return `<div class="empty">No Codex sessions were found for this workspace.</div>`;
...
export class UsageDashboard implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    if (this.snapshot) view.webview.html = this.render(this.snapshot, view.webview, true);
  }

  show(snapshot: UsageSnapshot): void {
    this.snapshot = snapshot;
    if (!this.panel) {
      this.panel = vscode.window.createWebviewPanel("codexUsage.dashboard", "Codex Usage", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
  ...
  update(snapshot: UsageSnapshot): void {
    this.snapshot = snapshot;
    if (this.panel) this.panel.webview.html = this.render(snapshot, this.panel.webview, false);
    if (this.view) this.view.webview.html = this.render(snapshot, this.view.webview, true);
  }
  ...
  private configure(webview: vscode.Webview): void {
    ...
    webview.onDidReceiveMessage(async (message) => {
      if (message?.command === "refresh") await vscode.commands.executeCommand("codexUsage.refresh");
      if (message?.command === "openFull") await vscode.commands.executeCommand("codexUsage.openDashboard");
      if (message?.command === "export" && typeof message.sessionId === "string") await vscode.commands.executeCommand("codexUsage.exportSession", message.sessionId);
    });
  }

  private render(snapshot: UsageSnapshot, webview: vscode.Webview, compact: boolean): string {
    const logo = webview.asWebviewUri(vscode.Uri.joinPath(this.extensionUri, "media", "codex-logo-gray.png"));
    ...
    const projectTokens = snapshot.sessions.reduce((sum, item) => sum + item.tokenUsage.totalTokens, 0);
    const limitCards = buckets.map((bucket) => `<section class="section limit">
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>
      ...
      <div class="grid"><div class="stat"><small>Plan</small><b>${escapeHtml(snapshot.account?.planType ?? snapshot.account?.type ?? "Offline")}</b><span>Codex account</span></div><div class="stat"><small>Today</small><b>${formatNumber(todayTokens)}</b><span>account tokens</span></div><div class="stat"><small>Project</small><b>${formatNumber(projectTokens)}</b><span>${snapshot.sessions.length} sessions</span></div><div class="stat"><small>Lifetime</small><b>${formatNumber(summary?.lifetimeTokens)}</b><span>Codex tokens</span></div></div>
      ...
      <script>const vscode=acquireVsCodeApi();function refresh(){vscode.postMessage({command:'refresh'})}function openFull(){vscode.postMessage({command:'openFull'})}function exportSession(id){vscode.postMessage({command:'export',sessionId:id})}</script>
    </main></body></html>`;
  }
}
